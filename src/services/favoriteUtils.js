...
export const addToFavorites = async (deal) => {
  ...
  if (token) {
    ...
    try {
      const response = await fetch(`${API_URL}/favorites/`, { // ✅ Dynamic URL
        method: 'POST',
        headers: {
...
export const removeFromFavorites = async (dealId) => {
  ...
  if (token) {
    ...
    try {
      const response = await fetch(`${API_URL}/favorites/${dealId}`, { // ✅ Dynamic URL
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
...
export const syncGuestFavoritesToAccount = async (token) => {
  ...
  for (const favorite of guestFavorites) {
    try {
      await fetch(`${API_URL}/favorites/`, { // ✅ Dynamic URL
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
