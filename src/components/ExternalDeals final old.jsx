...
export default function ExternalDeals() {
  ...
  const fetchExternalDeals = async () => {
    ...
    try {
      ...
      switch(activeSource) {
        case 'all':
          // Fetch from BOTH sources for "All Deals"
          try {
            const [slickdealsRes, dealnewsRes] = await Promise.all([
            ...
            const dealnewsData = dealnewsRes.ok ? await dealnewsRes.json() : { deals: [] };
            
            const slickdeals = extractDealsArray(slickdealsData);
            const dealnews = extractDealsArray(dealnewsData);
            
            // Combine and take first 6 (3 from each)
            const combined = [
              ...slickdeals.slice(0, 3),
              ...dealnews.slice(0, 3)
            ];
            
            console.log('All Deals - Slickdeals:', slickdeals.length, 'DealNews:', dealnews.length);
            setDeals(combined);
            setLoading(false);
            return;
          } catch (error) {
        ...
        case 'slickdeals':
          ...
          break;
        case 'dealnews':
          endpoint = 'http://localhost:8000/api/v1/external/deals/rapidapi'; // Still uses rapidapi endpoint
          break;
        case 'coupons':
      ...
      if (!response.ok) {
        ...
        if (activeSource !== 'slickdeals' && activeSource !== 'all') {
  ...
  const getSourceBadge = (source) => {
    const badges = {
      ...
      dealnews: { bg: 'bg-orange-600', text: 'REDDIT', icon: '🤖' },
      rapidapi: { bg: 'bg-orange-600', text: 'REDDIT', icon: '🤖' },
  ...
  // Image loading helper - tries direct loading first
  const getProxiedImageUrl = (imageUrl) => {
    ...
    // Try direct loading first (Slickdeals CDN might allow it)
    return imageUrl;
    
    // If direct loading doesn't work, uncomment the line below to use proxy:
    // return `http://localhost:8000/api/v1/external/proxy-image?url=${encodeURIComponent(imageUrl)}`;
    
    // Or use public CORS proxy:
    ...
    <section className="py-20 bg-white">
      <div className="container mx-auto px-4">
        ...
        <div className="flex justify-center gap-3 mb-12 flex-wrap">
          ...
          <button
            onClick={() => setActiveSource('dealnews')}
            className={`px-6 py-3 rounded-xl font-semibold transition-all duration-200 ${
              activeSource === 'dealnews'
