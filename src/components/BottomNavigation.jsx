...
// Mobile-only bottom navigation bar with smart profile routing
...
export default function BottomNavigation() {
  ...
  // Helper function to get the correct profile/dashboard link
  const getProfilePath = () => {
    if (!user) return '/profile'; // Guest users go to profile (which shows login)
    
    // Logged in users go to their appropriate dashboard
    if (user.is_admin) return '/admin/dashboard';
    if (user.is_business_owner) return '/business/dashboard';
    return '/dashboard';
  };
  ...
  const navItems = [
    ...
    },
    {
      ...
      label: 'Profile' // Always shows "Profile"
    }
  ];
